...
//     function getAccountLiquidity(address) external view returns (uint, uint);
...
// contract CompoundLens is ComptrollerInterface, Reward {
...
//     function getCompSpeeds(Reward rewarder, CToken cToken) internal returns (uint, uint) {
...
//                 abi.encodePacked(
//                     rewarder.compSupplySpeeds.selector,
...
//             address(rewarder).call(
//                 abi.encodePacked(
//                     rewarder.compBorrowSpeeds.selector,
//                     abi.encode(address(cToken))
//                 )
//             );
...
//         ComptrollerInterface comptroller = ComptrollerInterface(address(cToken.comptroller()));
...
//             reserveFactorMantissa: cToken.reserveFactorScaled(),
...
//     function getAccountLimits(ComptrollerInterface comptroller, address account) public returns (AccountLimits memory) {
...
//     function getCompBalanceMetadataExt(Comp comp, ComptrollerInterface comptroller, address account) external returns (CompBalanceMetadataExt memory) {
