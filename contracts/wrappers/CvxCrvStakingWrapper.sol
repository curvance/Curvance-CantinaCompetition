...
//other considerations: might be worth refactoring to use earned() during checkpoints instead of
//claiming rewards each time
    ...
    function _calcCvxIntegral(
        ...
        for (uint256 u = 0; u < _accounts.length; u++) {
            ...
            if (_isClaim || userI < cvx_reward_integral) {
                uint256 receiveable = cvx_claimable_reward[_accounts[u]].add(
                    _balances[u].mul(cvx_reward_integral.sub(userI)).div(1e20)
                );
                if (_isClaim) {
                    if (receiveable > 0) {
    ...
    function earned(address _account) external view returns (EarnedData[] memory claimable) {
        ...
        for (uint256 i = 0; i < rewardCount; i++) {
            ...
            if (reward.reward_token == crv) {
                claimable[rewardCount].amount = cvx_claimable_reward[_account].add(
