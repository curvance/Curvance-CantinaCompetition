...
library PendlePtOracleLib {
    ...
    function getPtToAssetRate(
        ...
        uint256 assetToPtRate = uint256(
            MarketMathCore._getExchangeRateFromImpliedRate(
                lnImpliedRate,
                timeToExpiry
            )
        );
    ...
    function _getMarketLnImpliedRate(
        IPMarket market,
        uint32 duration
        ...
        return
            (lnImpliedRateCumulative[1] - lnImpliedRateCumulative[0]) /
            duration;
    }

    function _getPtToAssetRatePostExpiry(
        IPMarket market
    ) private view returns (uint256) {
