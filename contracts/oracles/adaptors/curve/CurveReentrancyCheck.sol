...
interface ICurveHackyPool {
    ...
    function remove_liquidity(
        uint256 _tokenAmount,
        uint256[6] calldata _amounts
    ) external view;

    function remove_liquidity(
        uint256 _tokenAmount,
        uint256[7] calldata _amounts
    ) external view;
...
abstract contract CurveReentrancyCheck {
    enum N_COINS {
        // solhint-disable-line contract-name-camelcase
        NOT_CONFIGURED,
        INVALID,
        TWO_COINS,
        THREE_COINS,
    ...
    uint256 public constant MIN_GAS_LIMIT = 6500;
    ...
    event ReentrancyCheckConfigUpdated(
        address _pool,
        uint256 _gasLimit,
        N_COINS _nCoins
    );
    ...
    function isLocked(address _pool) public view virtual returns (bool) {
        ...
        if (config.nCoins == N_COINS.TWO_COINS) {
            ...
            try
                pool.remove_liquidity{ gas: config.gasLimit }(0, amounts)
            {} catch (bytes memory) {}
