...
contract LendTrollerEntropy is MockLendTrollerMultiMarket {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _noOfTokens; i++) {
            ...
            cTokensUnderlyingAgg[i] = _deployPriceRouterForToken(
                address(cToken)
            );
            if (_entropy > 0) {
                console2.log("a %s", i);
    ...
    function _setCollateralDataWithEntropy(
        ...
        if (
            collReqB <=
            400 + (lendtroller.MIN_EXCESS_COLLATERAL_REQUIREMENT() / 10 ** 14)
        ) {
            collReqB =
                400 +
                (lendtroller.MIN_EXCESS_COLLATERAL_REQUIREMENT() / 10 ** 14) +
                1;
        }
    ...
    function _executeBorrows(
        ...
        for (uint256 i = 0; i < noOfUsersCollateral; i++) {
            ...
            while (true) {
                ...
                (accCollateral, accMaxDebt, accDebt) = lendtroller.statusOf(
                    users[i]
                );
