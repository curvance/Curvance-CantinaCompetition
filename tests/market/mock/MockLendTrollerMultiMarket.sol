...
contract MockLendTrollerMultiMarket is TestBaseMarket {
    ...
    function _deployPriceRouterForToken(
        address token
    ...
    {
        (liqAmount, liquidatedTokens, protocolTokens) = lendtroller
            .canLiquidate(
                address(_dToken),
                address(_cToken),
                _user,
        ...
        )
    {
        ...
        ) = lendtroller.tokenData(address(_cToken));
    ...
    {
        (overR, underR) = lendtroller.hypotheticalLiquidityOf(
            account,
            address(mTokenModified),
            redeemTokens,
            0
        ...
        (overB, underB) = lendtroller.hypotheticalLiquidityOf(
            account,
            address(mTokenModified),
            0,
            borrowAmount
