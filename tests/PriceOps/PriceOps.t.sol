...
contract PriceOpsTest is TestBase {
    ...
    address private STETH_USD_FEED =
        0xCfE54B5cD566aB89272946F602D76Ea879CAb4a8;
    address private STETH_ETH_FEED =
    ...
    // Uniswap V3 Pools
    address private WBTC_WETH_05_POOL =
        0x4585FE77225b41b697C938B018E2Ac67Ac5a20c0;
    address private FRAX_USDC_05_POOL =
    ...
    ICurvePool stEthEth =
        ICurvePool(payable(0xDC24316b9AE028F1497c275EB9192a3Ea0f67022));
    ...
    function setUp() public {
        ...
        WBTC_PRICE_ETH = uint256(
            IChainlinkAggregator(WBTC_ETH_FEED).latestAnswer()
        );
        USDC_PRICE_USD = uint256(
            IChainlinkAggregator(USDC_USD_FEED).latestAnswer()
        );
        FRAX_PRICE_USD = uint256(
            IChainlinkAggregator(FRAX_USD_FEED).latestAnswer()
        );
    ...
    function testEditAssetFlow() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PriceOps.PriceOps__AssetAlreadyExists.selector,
                USDC
            )
        );
    ...
    function testChainlinkEthSource() public {
        ...
        (uint256 answer, , uint8 errorCode) = priceOps
            .getPriceInBaseEnforceNonZeroLower(STETH);

        assertApproxEqRel(
            answer,
            1 ether,
            0.02e18,
            "Answer should be approx equal to 1 ETH."
        );
    ...
    function testChainlinkUsdSource() public {
        (uint256 answer, , uint8 errorCode) = priceOps
            .getPriceInBaseEnforceNonZeroLower(USDC);
    ...
    function testTwapEthSource() public {
        ...
        (uint256 answer, , uint8 errorCode) = priceOps
            .getPriceInBaseEnforceNonZeroLower(WBTC);

        assertApproxEqRel(
            answer,
            WBTC_PRICE_ETH,
            0.01e18,
            "Answer should be approx equal to WBTC_PRICE_ETH."
        );
        assertEq(errorCode, 0, "There should be no error.");
    }
    ...
    function testTwapNonEthSource() public {
        ...
        (uint256 answer, , uint8 errorCode) = priceOps
            .getPriceInBaseEnforceNonZeroLower(FRAX);
    ...
    function testChainlinkSourceErrorCodes() public {
        ...
        assertEq(
            upper,
            lower,
            "Upper and lower should be equal since USDC has a single source."
        );
        ...
        assertEq(
            upper,
            0,
            "USDC price should be zero since the source is bad."
        );
        assertEq(
            upper,
            lower,
    ...
    function testChainlinkSourcesErrorCodes() public {
        ...
        assertApproxEqRel(
            ...
            lower,
            0.01e18,
            "Upper and lower should be approx equal."
        );
        ...
        assertEq(
            upper,
            lower,
            "Upper and lower should be equal since primary is bad."
        );
        assertTrue(
            upper == originalUpper || upper == originalLower,
            "Values reported should be 1 of the original."
        );
        ...
        assertGt(
            upper,
            0,
            "USDC price upper should be nonzero since the source is bad."
        );
        assertGt(
            lower,
            0,
            "USDC price lower should be nonzero since the source is bad."
        );
        assertTrue(
            upper != lower,
            "Upper and lower should not be equal since USDC prices have diverged."
        );
