...
| `combineAllLocks_for_all_continuous_to_continuous_terminal_should_succeed` | All existing locks are continuous                                   | Tests the scenario where all existing locks are continuous and are being combined into a single continuous lock                                                  | The combined lock's amount matches the total amount of original locks, and the user points remain the same as there's no turnaround from non-continuous to continuous. |
| `combineAllLocks_non_continuous_to_continuous_terminals_should_succeed`    | Mix of continuous and non-continuous locks (or no continuous locks) | Tests scenario where a user has a mix of continuous and non-continuous locks and they are being combined into a single continuous lock                           | The combined lock's amount matches the total amount of original locks, and the user points are correctly increased to account for the conversion to continuous lock.   |
| `combineAllLocks_should_succeed_to_non_continuous_terminal`                | Existing locks are both continuous and non-continuous               | Tests the scenario where a user combines all of their locks (either continuous, non-continuous, or a mix of both) and the result is a single non-continuous lock | The combined lock's amount matches the total amount of original locks, and the user points are correctly adjusted down if any lock was originally continuous.          |
