...
contract ConvexPositionVaultTest is TestBase {
    ...
    address private ETH_FAST_GAS_FEED =
    ...
    function setUp() public {
        ...
        uint256 price = uint256(
            IChainlinkAggregator(USDT_USD_FEED).latestAnswer()
        );
        ...
        uint256 vp = ICurvePool(curve3CryptoPool)
            .get_virtual_price()
            .changeDecimals(18, 8);
        PriceRouter.VirtualPriceBound memory vpBound = PriceRouter
            .VirtualPriceBound(
                uint96(vp),
                0,
                uint32(1.01e8),
        ...
        vp = ICurvePool(curve3CrvPool).get_virtual_price().changeDecimals(
            18,
            8
        );
        vpBound = PriceRouter.VirtualPriceBound(
            uint96(vp),
            0,
            uint32(1.01e8),
            uint32(0.99e8),
        ...
        cvxPositionTriCrypto = new ConvexPositionVault(
            CRV_3_CRYPTO,
            address(this),
            "Tri Crypto Vault",
            "TCV",
        ...
        {
            ConvexPositionVault.CurveDepositParams
                memory depositParams = ConvexPositionVault.CurveDepositParams(
                    WETH,
                    3,
                    2,
                    false,
            ...
            ConvexPositionVault.CurveSwapParams[]
                memory swapsToETH = new ConvexPositionVault.CurveSwapParams[](
                    2
                );
            ...
            swapsToETH[0] = ConvexPositionVault.CurveSwapParams(
                route0,
                swaps,
                CRV,
                1e8
            ...
            swapsToETH[1] = ConvexPositionVault.CurveSwapParams(
                route1,
                swaps,
                CVX,
                1e8
        ...
        cvxPosition3Pool = new ConvexPositionVault(
            CRV_DAI_USDC_USDT,
            address(this),
            "3 Pool Vault",
            "3PV",
...

        {
            ...
            swapsToTarget = ConvexPositionVault.CurveSwapParams(
                route,
                swaps0,
                WETH,
                0
    ...
    function testConvexPositionVault3Pool() public {
        ...
        assertEq(
            ...
            shareValue,
            "Share Price should be constant through harvest."
        );
        ...
        cvxPosition3Pool.withdraw(
            cvxPosition3Pool.totalAssets(),
            address(this),
            address(this)
        );
    }
    ...
    function testMultipleDepositors(uint256 assetsA, uint256 assetsB) public {
        ...
        assertEq(
            cvxPositionTriCrypto.maxWithdraw(userA),
            assetsA,
            "User should be able to withdraw their deposit."
        );
        assertEq(
            cvxPositionTriCrypto.maxWithdraw(userB),
            assetsB,
            "User should be able to withdraw their deposit."
        );
        ...
        assertEq(
            zeroYield,
            0,
            "Yield earned should be yield because previous rewards are vesting."
        );
        ...
        vm.stopPrank();
        assertEq(
            CRV_3_CRYPTO.balanceOf(userA),
            userAWithdraw,
            "User did not receive full withdraw."
        );
    ...
    function testAttackerGettingMoreOutThanTheyPutIn() public {
        ...
        (bool upkeepNeeded, bytes memory performData) = cvxPosition3Pool
        ...
        uint256 attackerNewBalance = cvxPosition3Pool.balanceOf(attacker) +
            10**cvxPosition3Pool.decimals();
        ...
        (upkeepNeeded, performData) = cvxPosition3Pool.checkUpkeep(
            abi.encode(0, "0x")
        );
        ...
        assertEq(
            cvxPosition3Pool.isShutdown(),
            true,
            "Vault should be shutdown."
        );
    }
